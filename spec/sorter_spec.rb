...
RSpec.describe ColorContrastCalc::Sorter do
  ...
  describe '.compose_function' do
    ...
    context 'when colors are represented in hsl' do
      compiler = Sorter::COMPARE_FUNCTION_COMPILERS[Sorter::KeyTypes::COMPONENTS]
      compare = compiler.compile('sLh')
    ...
    context 'when colors are represented in hex' do
      compiler = Sorter::COMPARE_FUNCTION_COMPILERS[Sorter::KeyTypes::HEX]
      compare = compiler.compile('sLh')
    ...
    describe '#compare_components' do
      ...
      context 'when color_order is rgb' do
        ...
        it 'expects to return 1 when [0, 165, 70] and [0, 70, 165] are passed' do
          expect(compiler.compare_components(color1, color3, order)).to be 1
        end

        it 'expects to return 0 when [0, 165, 70] and [0, 165, 70] are passed' do
          expect(compiler.compare_components(color1, color1, order)).to be 0
        end
      end
      ...
      context 'when color_order is Rgb' do
        ...
        it 'expects to return 1 when [165, 70, 0] and [0, 165, 70] are passed' do
          expect(compiler.compare_components(color2, color1, order)).to be(-1)
        end
      ...
      context 'when color_order is gBr' do
        ...
        it 'expects to return 1 when [165, 70, 0] and [0, 70, 165] are passed' do
          expect(compiler.compare_components(color2, color3, order)).to be 1
        end

        it 'expects to return 1 when [0, 70, 165] and [165, 70, 0] are passed' do
          expect(compiler.compare_components(color3, color2, order)).to be(-1)
        end
  ...
  describe '[COMPONENTS].compile' do
    compiler = Sorter::COMPARE_FUNCTION_COMPILERS[Sorter::KeyTypes::COMPONENTS]
  ...
  describe '[COLOR].compile -- rgb' do
    compiler = Sorter::COMPARE_FUNCTION_COMPILERS[Sorter::KeyTypes::COLOR]
